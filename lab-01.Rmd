...
there are or is entry include NA
...
visitors_with_nations %>% 
  filter(nation == "Scotland", admission == "Free") %>%
  summarise(count = n(), prportion = n() / 97)

visitors_with_nations %>% 
  filter(nation == "Wales", admission == "Free") %>%
  summarise(count = n(), prportion = n() / 2)

visitors_with_nations %>% 
  filter(nation == "Northern Ireland", admission == "Free") %>%
  summarise(count = n(), prportion = n() / 9)
            
visitors_with_nations %>% 
  filter(nation == "England", admission == "Free") %>%
  summarise(count = n(), prportion = n() / 240)
...
visitors_with_nations %>%
  ...
  group_by(admission) %>%
  summarise(sum_2022 = sum(n_2022), sum_2021 = sum(n_2021, na.rm = TRUE)) %>%
  mutate(change = (sum_2022 - sum_2021) / sum_2021) %>%
  arrange(change)
